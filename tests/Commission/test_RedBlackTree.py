...
class TestRBTree:
    ...
    @staticmethod
    def build_random_RBTree(size: int = random.randint(0, 10000), a=-int(1e6), b=int(1e6)) -> RBTree:
        new_RBTree = RBTree()
        for i in range(size):
    ...
    def check_tree_invariant(self, tree: RBTree) -> bool:
        def _recursion_check(curr_root: Node) -> bool:
            ...
            if curr_root == NULL or curr_root.left == NULL and curr_root.right == NULL:
                return True
